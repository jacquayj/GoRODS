...
/**************************************************************************
  ...
  This file contains helper functions that can be used by the icat
  routines.  Many of these are currently unused and are now ifdef'ed
  out (actually, via '#if 0') so that a test coverage analysis tool
  (gcov) will ignore them.  They can be easily built if/when we'd
  like to use them.  These were written early in the ICAT development
